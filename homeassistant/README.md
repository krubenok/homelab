...
## Native GitOps deploy target
- Use Home Assistant official add-on `core_git_pull`.
- Recommended deployment branch should contain `/config` files at repository root.
- Keep add-on `git_command: pull` (not `reset`) so untracked local secret files remain intact.
...
4. On HA, run a pull:
   - start the `core_git_pull` add-on (manual) to update `/config` from the deploy branch.
